...
from vllm._atom.utils import mark_spliting_op
...
class Attention:
    def __new__(cls, *args, **kwargs):
        ...
        if is_vllm():
            from vllm._atom.plugin.vllm.attention.layer import AttentionForVllm

            return AttentionForVllm(*args, **kwargs)
        if is_sglang():
            from vllm._atom.plugin.sglang.attention import AttentionForSGLang

            return AttentionForSGLang(*args, **kwargs)

        from vllm._atom.model_ops.paged_attention import Attention as AttentionForAtom
