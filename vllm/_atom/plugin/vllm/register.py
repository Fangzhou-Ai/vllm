...
ATOM_CAUSAL_LM_MODEL_WRAPPER = "vllm._atom.plugin.vllm.model_wrapper:ATOMForCausalLM"
ATOM_MOE_CAUSAL_LM_MODEL_WRAPPER = "vllm._atom.plugin.vllm.model_wrapper:ATOMMoEForCausalLM"

# when register new model to vllm, add here
# Keys is from hf config arch name
# This vendored port targets DeepSeek-V4 only (+ its MTP draft). Other ATOM
# model wrappers were dropped during the vLLM port cleanup; re-add their entries
# and source files to re-enable them.
...
def register_platform() -> Optional[str]:
    ...
    return "vllm._atom.plugin.vllm.platform.ATOMPlatform"
