...
def use_aiter_mhc() -> bool:
    ...
    Requires both the master ``VLLM_ROCM_USE_AITER`` switch and the
    ``VLLM_ROCM_USE_AITER_MHC`` sub-switch to be enabled. The aiter mHC
    kernels need aiter >= 0.1.14, which contains the sqrsum race-condition
    fix in ``mhc_pre_gemm_sqrsum_kernel`` (without it the result is wrong at
    large token counts).
    """
    return envs.VLLM_ROCM_USE_AITER and envs.VLLM_ROCM_USE_AITER_MHC
