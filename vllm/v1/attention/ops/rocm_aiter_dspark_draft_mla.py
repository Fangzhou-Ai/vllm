...
The kernel itself lives in aiter (``module_kimi_k3_mla``), which also owns
...
def dspark_draft_mla_decode(
    ...
    try:
        from aiter import dspark_draft_mla_decode as _aiter_draft_mla
    except ImportError:
        return False
    ...
    _aiter_draft_mla(
        ...
        seq_lens,
        query_len,
        # Caps the split count, so it wants this batch's longest KV span --
        # passing the whole cache's extent would leave the cap open.
        max_seq_len,
        sm_scale,
        _scalar(q_scale),
        _scalar(kv_scale),
