...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Select the mHC path:
        # - aiter unfused pre/post (default on ROCm under VLLM_ROCM_USE_AITER)
        #   or the torch fallback when tilelang is unavailable ->
        #   _forward_unfused_post_pre
        # - tilelang fused post+pre (CUDA, or ROCm when VLLM_ROCM_USE_TILELANG_MHC
        #   opts in) -> _forward_fused_post_pre
        if not self.has_tilelang or (current_platform.is_rocm() and use_aiter_mhc()):
