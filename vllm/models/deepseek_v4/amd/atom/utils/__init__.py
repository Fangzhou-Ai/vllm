...
def is_torch_equal_or_newer(target: str) -> bool:
    ...
    Vendored from ATOM ``atom/utils/__init__.py`` (only consumer is the MoE
    ``fused_moe/utils.disable_inplace`` helper).
    """
    ...
    try:
        return _pkg_version.parse(str(torch.__version__)) >= _pkg_version.parse(
            target
        )
    except Exception:
