...
class Config:
    ...
    def __init__(self):
        ...
        # MoE single-node (non-EP, non-DP) knobs read by the ported ATOM
        # FusedMoE stack (model_ops/moe.py, topK.py). Kept False/plain-TP so
        # FusedMoEParallelConfig.make collapses to pure tensor-parallel sharding
        # and shared-expert fusion stays enabled.
        self.enable_dp_attention = False
        self.moe_ep_flatten_tp_across_dp = False
        self.enable_expert_parallel = False
        # topK metadata buffers + FusedMoEConfig size to this; overridden from
        # vllm_config.scheduler_config.max_num_batched_tokens in
        # setup_atom_config_and_args. Default covers the bench (16384).
        self.max_num_batched_tokens = 32768
        self.tensor_parallel_size = 1
        # Fallback only; MoE always passes the layer quant_config explicitly.
