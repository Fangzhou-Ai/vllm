...
def set_weight_attrs(weight, weight_attrs):
    ...
    Vendored from ATOM ``atom/model_loader/weight_utils.py`` (used by the
    ported FusedMoE ``create_weights`` to tag expert params with their
    weight_loader). The full ATOM weight_utils drags huggingface_hub/filelock,
    so only this function is vendored here.
    """
