...
pytestmark = pytest.mark.skipif(
    ...
    reason="Kimi-K3 DSpark draft MLA decode requires gfx950",
)
...
def test_dspark_draft_mla_matches_reference(
...
def test_dspark_draft_mla_declines_unsupported_page_size():
    from vllm.v1.attention.ops.rocm_aiter_dspark_draft_mla import (
        dspark_draft_mla_decode,
    )
    ...
    assert not dspark_draft_mla_decode(q, kv, out, bt, sl, 7, 100, 0.1, 1.0, 1.0)


def test_aiter_mla_backend_declares_non_causal_support():
    """The draft group reaches the kernel through this declaration."""
